...
async function updatePaymentSettings() {
    ...
    try {
        // FIXED: Added /api prefix
        const result = await apiCall('/api/admin/settings/payment', 'POST', {
...
async function updateSystemSettings() {
    ...
    try {
        // FIXED: Added /api prefix
        const result = await apiCall('/api/admin/update-credentials', 'POST', data);
...
function resetPaymentSettings() {
    showConfirmModal('Reset all payment settings to default values?', async () => {
        try {
            // FIXED: Added /api prefix
            const result = await apiCall('/api/admin/settings/reset', 'POST');
            if (result.success) {
...
async function clearAllData() {
    showConfirmModal('WARNING: This will delete ALL employee data! This action cannot be undone. Are you absolutely sure?', async () => {
        try {
            // FIXED: Added /api prefix
            const result = await apiCall('/api/admin/clear-data', 'POST', { confirm: true });
            if (result.success) {
...
async function loadSystemInfo() {
    try {
        ...
        const backupInfo = await apiCall('/api/admin/backup-info');
...
async function saveSessionSettings() {
    ...
    try {
        // FIXED: Added /api prefix
        const result = await apiCall('/api/admin/session-settings', 'POST', {
...
async function clearAllSessions() {
    showConfirmModal('Clear all active admin sessions? Users will need to login again.', async () => {
        try {
            // FIXED: Added /api prefix
            const result = await apiCall('/api/admin/clear-sessions', 'POST');
            if (result.success) {
...
async function resetSystem() {
    showConfirmModal('WARNING: This will reset the ENTIRE system to factory settings. All employees, attendance records, and settings will be lost. This action CANNOT be undone. Type "RESET" to confirm.', async () => {
        ...
        if (confirmText === 'RESET') {
            try {
                // FIXED: Added /api prefix
                const result = await apiCall('/api/admin/factory-reset', 'POST', { confirm: true });
                if (result.success) {
